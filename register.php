...
foreach ($toolproxies as $key => $toolproxy) {
    if (($toolproxy->state == QTYPE_LTI_TOOL_PROXY_STATE_PENDING) || ($toolproxy->state == QTYPE_LTI_TOOL_PROXY_STATE_ACCEPTED)) {
...
$redirect = new moodle_url('/question/type/lti/toolproxies.php');
if ($duplicate) {
    redirect($redirect,  get_string('duplicateregurl', 'qtype_lti'));
}


$profileservice = qtype_lti_get_service_by_name('profile');
if (empty($profileservice)) {
    redirect($redirect,  get_string('noprofileservice', 'qtype_lti'));
}

$url = new moodle_url('/question/type/lti/register.php', array('id' => $id));
$PAGE->set_url($url);

admin_externalpage_setup('ltitoolproxies');


$PAGE->set_heading(get_string('toolproxyregistration', 'qtype_lti'));
$PAGE->set_title("{$SITE->shortname}: " . get_string('toolproxyregistration', 'qtype_lti'));

// Print the page header.
echo $OUTPUT->header();

echo $OUTPUT->heading(get_string('toolproxyregistration', 'qtype_lti'));

echo $OUTPUT->box_start('generalbox');

// Request the registration request content with an object tag.
$registration = new moodle_url('/question/type/lti/registration.php',
...
echo get_string('register_warning', 'qtype_lti');
echo "\n</p>\n";
...
$resize = '
        ...
        //<![CDATA[
            ...
            function doOnload() {
                window.clearTimeout(qtype_lti_timer);
            }
            var qtype_lti_timer = window.setTimeout(doReveal, 20000);
