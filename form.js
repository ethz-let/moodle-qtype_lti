...
(function () {
    ...
    M.qtype_lti = M.qtype_lti || {};

    M.qtype_lti.QTYPE_LTI_SETTING_NEVER = 0;
    M.qtype_lti.QTYPE_LTI_SETTING_ALWAYS = 1;
    M.qtype_lti.QTYPE_LTI_SETTING_DELEGATE = 2;
    ...
    M.qtype_lti.editor = {
        init : function (yui3, settings) {
            ...
            typeSelector.on('change',
                    function (e) {
                        // Reset configuration fields when another preconfigured
                        // tool is selected.
                        ...
                        if (self.getSelectedToolTypeOption().getAttribute(
                                'toolproxy')) {
                            var allowname = Y
                                    .one('#id_instructorchoicesendname');
                            allowname.set('checked', !self
                                    .getSelectedToolTypeOption().getAttribute(
                                            'noname'));

                            var allowemail = Y
                                    .one('#id_instructorchoicesendemailaddr');
                            allowemail.set('checked', !self
                                    .getSelectedToolTypeOption().getAttribute(
                                            'noemail'));

                            var allowgrades = Y
                                    .one('#id_instructorchoiceacceptgrades');
                            allowgrades.set('checked', !self
                                    .getSelectedToolTypeOption().getAttribute(
                                            'nogrades'));
                        }
                    });
            ...
            var contentItemUrl = contentItemButton
                    .getAttribute('data-contentitemurl');
            ...
            var textAreas = new Y.NodeList([ Y.one('#id_toolurl'),
                    Y.one('#id_securetoolurl'), Y.one('#id_resourcekey'),
                    Y.one('#id_password') ]);
            ...
            textAreas.on('keyup', function (e) {
                ...
                // If no more changes within 2 seconds, look up the matching
                // tool URL.
        ...
        updateAutomaticToolMatch : function (field) {
            ...
            if (!automatchToolDisplay) {
                automatchToolDisplay = Y.Node.create('<span />').set('id', id)
            ...
            var selectedOption = typeSelector.one('option[value="'
                    + selectedToolType + '"]');
            ...
            if (selectedToolType > 0) {
                // If the entered domain matches the domain of the tool
                // configuration.
            ...
            // We still check the Launch URL with the server as course/site
            ...
            var continuation = function (toolInfo, inputfield) {
                /*
                 * if (inputfield === undefined || (inputfield.get('id') !=
            ...
            if (selectedToolType && self.toolTypeCache[selectedToolType]) {
                ...
                self.findToolByUrl(url, selectedToolType, function (toolInfo) {
                    if (toolInfo) {
                        // Cache the result based on whether the URL or tool
                        // type was used to look up the tool.
                        if (!selectedToolType) {
        ...
        /**
         * Updates display of privacy settings to show course / site tool
         * configuration settings.
         */
        updatePrivacySettings : function (toolInfo) {
            if (!toolInfo || !toolInfo.toolid) {
                toolInfo = {
                    sendname : M.qtype_lti.QTYPE_LTI_SETTING_DELEGATE,
                    sendemailaddr : M.qtype_lti.QTYPE_LTI_SETTING_DELEGATE,
                    acceptgrades : M.qtype_lti.QTYPE_LTI_SETTING_DELEGATE
                }
            }
            ...
            // Store a copy of user entered privacy settings as we may overwrite
            // them.
            ...
            for (setting in privacyControls) {
                if (privacyControls.hasOwnProperty(setting)) {
                    ...
                    // Only store the value if it hasn't been forced by the
                    // editor.
                    if (!control.get('disabled')) {
                        this.userPrivacySettings[setting] = control
                                .get('checked');
                    }
                }
            }
            ...
            for (setting in privacyControls) {
                if (privacyControls.hasOwnProperty(setting)) {
                    ...
                    if (settingValue == M.qtype_lti.QTYPE_LTI_SETTING_NEVER) {
                        control.set('disabled', true);
                        control.set('checked', false);
                        control.set('title', M.util.get_string('forced_help',
                                'qtype_lti'));
                    } else if (settingValue == M.qtype_lti.QTYPE_LTI_SETTING_ALWAYS) {
                        control.set('disabled', true);
                        control.set('checked', true);
                        control.set('title', M.util.get_string('forced_help',
                                'qtype_lti'));
                    } else if (settingValue == M.qtype_lti.QTYPE_LTI_SETTING_DELEGATE) {
                        control.set('disabled', false);

                        // Get the value out of the stored copy.
                        control.set('checked',
                                this.userPrivacySettings[setting]);
                        control.set('title', '');
                    }
                }
            }
        ...
        getSelectedToolTypeOption : function () {
            ...
            return typeSelector.one('option[value="'
                    + typeSelector.get('value') + '"]');
        },
        ...
        addOptGroups : function () {
            ...
            if (typeSelector.one('option[courseTool=1]')) {
                ...
                var globalGroup = Y.Node.create('<optgroup />').set('id',
                        'global_tool_group').set('label',
                        M.util.get_string('global_tool_types', 'qtype_lti'));

                var courseGroup = Y.Node.create('<optgroup />').set('id',
                        'course_tool_group').set('label',
                        M.util.get_string('course_tool_types', 'qtype_lti'));

                var globalOptions = typeSelector.all('option[globalTool=1]')
                        .remove().each(function (node) {
                            globalGroup.append(node);
                        });

                var courseOptions = typeSelector.all('option[courseTool=1]')
                        .remove().each(function (node) {
        ...
        createTypeEditorButtons : function () {
            ...
            var createIcon = function (id, tooltip, iconUrl) {
                return Y.Node.create('<a />').set('id', id).set('title',
                        tooltip).setStyle('margin-left', '.5em').set('href',
                        'javascript:void(0);').append(
                        Y.Node.create('<img src="' + iconUrl + '" />'));
            }

            var addIcon = createIcon('lti_add_tool_type', M.util.get_string(
                    'addtype', 'qtype_lti'), this.settings.add_icon_url);
            var editIcon = createIcon('lti_edit_tool_type', M.util.get_string(
                    'edittype', 'qtype_lti'), this.settings.edit_icon_url);
            var deleteIcon = createIcon('lti_delete_tool_type', M.util
                    .get_string('deletetype', 'qtype_lti'),
                    this.settings.delete_icon_url);
            ...
            editIcon
                    .on(
                            ...
                            function (e) {
                                ...
                                if (self.getSelectedToolTypeOption()
                                        .getAttribute('editable')) {
                                    window
                                            .open(
                                                    self.settings.instructor_tool_type_edit_url
                                                            + '&action=edit&typeid='
                                                            + toolTypeId,
                                                    'edit_tool');
                                } else {
                                    alert(M.util.get_string('cannot_edit',
                                            'qtype_lti'));
                                }
                            });
            ...
            addIcon.on('click', function (e) {
                if (self.settings.can_add_course_tool == true) {
                    window.open(self.settings.instructor_tool_type_edit_url
                            + '&action=add', 'add_tool');
                } else {
            ...
            deleteIcon.on('click', function (e) {
                ...
                if (self.getSelectedToolTypeOption().getAttribute('editable')) {
                    if (confirm(M.util.get_string('delete_confirmation',
                            'qtype_lti'))) {
                        self.deleteTool(toolTypeId);
                    }
                } else {
                    alert(M.util.get_string('cannot_delete', 'qtype_lti'));
                }
            });
        ...
        toggleEditButtons : function () {
            ...
            // Does not work in older browsers, but alerts will catch those
            // cases.
        ...
        addToolType : function (toolType) {
            ...
            var option = Y.Node.create('<option />').set('text', toolType.name)
                    .set('value', toolType.id).set('selected', 'selected')
                    .setAttribute('editable', '1').setAttribute('courseTool',
                            '1').setAttribute('domain', toolType.tooldomain);
            ...
            // Adding the new tool may affect which tool gets matched
            // automatically.
            this.clearToolCache();
            ...
            require([ "core/notification" ], function (notification) {
                notification.addNotification({
                    message : M.util.get_string('tooltypeadded', 'qtype_lti'),
                    type : "success"
                });
            });
        },
        ...
        updateToolType : function (toolType) {
            ...
            var option = typeSelector
                    .one('option[value="' + toolType.id + '"]');
            ...
            require([ "core/notification" ], function (notification) {
                notification
                        .addNotification({
                            message : M.util.get_string('tooltypeupdated', 'qtype_lti'),
                            type : "success"
                        });
            });
        },
            ...
            Y.io(self.settings.instructor_tool_type_edit_url + '&action=delete&typeid=' + toolTypeId, {
                on : {
                    success : function () {
                        ...
                        // Editing the tool may affect which tool gets matched
                        // automatically.
                        self.clearToolCache();
                        self.updateAutomaticToolMatch(Y.one('#id_toolurl'));
                        self.updateAutomaticToolMatch(Y
                                .one('#id_securetoolurl'));

                        require([ "core/notification" ],
                                function (notification) {
                                    notification
                                            .addNotification({
                                                message : M.util.get_string('tooltypedeleted', 'qtype_lti'),
                                                type : "success"
                                            });
                                });
                    },
                    failure : function () {
                        require([ "core/notification" ],
                                function (notification) {
                                    notification.addNotification({
                                        message : M.util.get_string('tooltypenotdeleted', 'qtype_lti'),
                                        type : "problem"
                                    });
                                });
                    }
        ...
        /**
         * Gets the tool type ID of the selected tool that supports Content-Item
         * selection.
         *
         * @returns {number|boolean} The ID of the tool type if it supports
         *          Content-Item selection. False, otherwise.
         */
        ...
        resetToolFields : function () {
            ...
            var fields = Y
                    .all('#id_toolurl, #id_securetoolurl, #id_instructorcustomparameters, #id_icon, #id_secureicon');
