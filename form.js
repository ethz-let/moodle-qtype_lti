...
(function(){
    ...
    M.qtype_lti.editor = {
        init: function(yui3, settings){
            ...
            contentItemButton.on('click', function() {
                ...
                if (contentItemId) {
                    ...
                    require(['qtype_lti/contentitem'], function(contentitem) {
        ...
        updateAutomaticToolMatch: function(field){
            ...
            if(selectedToolType > 0){
                ...
                if(match && match[1] && match[1].toLowerCase() === selectedOption.getAttribute('domain').toLowerCase()){
                    automatchToolDisplay.set('innerHTML',  '<img style="vertical-align:text-bottom" src="' + self.settings.green_check_icon_url + '" />' + M.util.get_string('using_tool_configuration', 'qtype_lti') + selectedOption.get('text'));
                } else {
                    // The entered URL does not match the domain of the tool configuration
                    automatchToolDisplay.set('innerHTML', '<img style="vertical-align:text-bottom" src="' + self.settings.warning_icon_url + '" />' + M.util.get_string('domain_mismatch', 'qtype_lti'));
                }
            }
            ...
            if(key.get('value') !== '' && secret.get('value') !== ''){
                automatchToolDisplay.set('innerHTML',  '<img style="vertical-align:text-bottom" src="' + self.settings.green_check_icon_url + '" />' + M.util.get_string('custom_config', 'qtype_lti'));
            }
            ...
            var continuation = function(toolInfo, inputfield){
                ...
                */
                if(toolInfo.toolname){
                    automatchToolDisplay.set('innerHTML',  '<img style="vertical-align:text-bottom" src="' + self.settings.green_check_icon_url + '" />' + M.util.get_string('using_tool_configuration', 'qtype_lti') + toolInfo.toolname);
                } else if(!selectedToolType) {
                    // Inform them custom configuration is in use
                    if(key.get('value') === '' || secret.get('value') === ''){
                        automatchToolDisplay.set('innerHTML', '<img style="vertical-align:text-bottom" src="' + self.settings.warning_icon_url + '" />' + M.util.get_string('tool_config_not_found', 'qtype_lti'));
                    }
                }
                if (toolInfo.cartridge) {
                    automatchToolDisplay.set('innerHTML', '<img style="vertical-align:text-bottom" src="' + self.settings.green_check_icon_url +
                                             '" />' + M.util.get_string('using_tool_cartridge', 'qtype_lti'));
                }
            };
            ...
            for(setting in privacyControls){
                if(privacyControls.hasOwnProperty(setting)){
                    ...
                    if(settingValue == M.qtype_lti.QTYPE_LTI_SETTING_NEVER){
                        ...
                        control.set('title', M.util.get_string('forced_help', 'qtype_lti'));
        ...
        addOptGroups: function(){
            ...
            if(typeSelector.one('option[courseTool=1]')){
                ...
                var globalGroup = Y.Node.create('<optgroup />')
                                    ...
                                    .set('label', M.util.get_string('global_tool_types', 'qtype_lti'));

                var courseGroup = Y.Node.create('<optgroup />')
                                    .set('id', 'course_tool_group')
                                    .set('label', M.util.get_string('course_tool_types', 'qtype_lti'));
        ...
        createTypeEditorButtons: function(){
            ...
            var addIcon = createIcon('lti_add_tool_type', M.util.get_string('addtype', 'qtype_lti'), this.settings.add_icon_url);
            var editIcon = createIcon('lti_edit_tool_type', M.util.get_string('edittype', 'qtype_lti'), this.settings.edit_icon_url);
            var deleteIcon  = createIcon('lti_delete_tool_type', M.util.get_string('deletetype', 'qtype_lti'), this.settings.delete_icon_url);
            ...
            editIcon.on('click', function(e){
                ...
                if(self.getSelectedToolTypeOption().getAttribute('editable')){
                    ...
                    alert(M.util.get_string('cannot_edit', 'qtype_lti'));
                }
            });
            ...
            deleteIcon.on('click', function(e){
                ...
                if(self.getSelectedToolTypeOption().getAttribute('editable')){
                    if(confirm(M.util.get_string('delete_confirmation', 'qtype_lti'))){
