...
Feature: Restoring Moodle 2 backup restores qtype LTI configuration
  ...
  Scenario: Backup and restore course with preconfigured site LTI tool on the same site
    ...
    And I navigate to "Manage tools" node in "Site administration > Plugins > Question Types > LTI (ETH) > External tool"
  ...
  Scenario: Backup and restore course with preconfigured course LTI tool on the same site
    ...
    # In the first course create an LTI qtype that uses a course preconfigured tool
    And I add a "External tool"
    And I set the following fields to these values:
      | Question name | Test tool qtype 2 |
    And I follow "Add preconfigured tool"
    And I switch to "add_tool" window
    And I set the field "Tool name" to "My qtype tool"
    ...
    And I open "Test tool qtype 2" actions menu
    And I choose "Edit settings" in the open action menu
