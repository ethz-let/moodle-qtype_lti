...
Feature: Content-Item support
  In order to easily add qtype and content in a course from an LTI external tool
  As a teacher
  ...
  Background:
    ...
    And I set the field "Tool configuration usage" to "Show in qtype chooser and as a preconfigured tool"
  ...
  Scenario: Editing a tool's settings that was configured from a preconfigured tool that supports Content-Item.
    ...
    And I add a "Teaching Tool 1"
    And the "Select content" "button" should be enabled
    And I set the field "Qtype name" to "Test tool qtype 1"
    And I expand all fieldsets
    And I set the field "Launch container" to "Embed"
    And I press "Save and return to course"
    And I open "Test tool qtype 1" actions menu
