...
/**
 * Controls all of the behaviour and interaction with a tool type card. These
 * are listed on the LTI tool type management page.
 *
 * See template: qtype_lti/tool_card
 *
 * @module qtype_lti/tool_card_controller
 ...
 * @since 3.1
 */
define(['jquery', 'core/ajax', 'core/notification', 'core/templates', 'qtype_lti/tool_type', 'qtype_lti/events', 'qtype_lti/keys',
        'core/str'],
        function($, ajax, notification, templates, toolType, ltiEvents, KEYS, str) {
             ...
             * @private
             * @param {JQuery}
             *            element jQuery object representing the tool card.
            ...
            /**
             * Return the element that contains the capabilities approval for the user.
             *
            ...
            /**
             * Checks if the tool type has capabilities that need approval. If it does
             * then the container will be present.
             *
            ...
            /**
             * Show the success announcement. The announcement is only visible for 2
             * seconds.
             *
            ...
            /**
             * Show the failure announcement. The announcement is only visible for 2
             * seconds.
             *
            ...
            /**
             * Delete the tool type from the Moodle server. Triggers a success or
             * failure announcement depending on the result.
             *
             * @method deleteType
             * @private
             * @param {JQuery}
                                    ...
                                    toolType.delete(typeId)
                                        .done(function() {
                                                ...
                                                announceSuccess(element)
                                                    ...
                                                    .always(function() {
                                                            // Always resolve because
                                                            // even if the announcement
                                                            // fails the type was
                                                            // deleted.
                                                            promise.resolve();
                                                    });
             ...
             * @private
             * @param {JQuery}
             *            element jQuery object representing the element.
             * @param {String}
             *            value to be saved.
             */
            ...
            /**
             * Send a request to update the description value for this tool in the
             * Moodle server.
             *
            ...
            /**
             * Send a request to update the name value for this tool in the Moodle
             * server.
             *
             * @method updateName
             * @private
            ...
            /**
             * Send a request to update the state for this tool to be configured
             * (active) in the Moodle server. A success or failure announcement is
            ...
            var setStatusActive = function(element) {
                ...
                promise.then(function(toolTypeData) {
                    ...
                    return templates.render('qtype_lti/tool_card', toolTypeData);
            ...
            var registerEventListeners = function(element) {
                ...
                descriptionElement.focus(function(e) {
                    ...
                    // we can check if the user has changed it before sending a request
                    // to
                    // the server.
            ...
            return /** @alias module:qtype_lti/tool_card_controller */ {
