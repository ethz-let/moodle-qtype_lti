...
$pageurl = new moodle_url('/question/type/lti/externalregistrationreturn.php');
...
if ($status !== 'success' && empty($err)) {
    ...
    if (!empty($msg)) {
        ...
        $err = get_string('failedtocreatetooltype', 'qtype_lti');
    }
}
$params = array('message' => s($msg), 'error' => s($err), 'id' => $id, 'status' => s($status));

$page = new \qtype_lti\configoutput\external_registration_return_page();
echo $output->render($page);

$PAGE->requires->js_call_amd('qtype_lti/external_registration_return', 'init', $params);
echo $output->footer();
