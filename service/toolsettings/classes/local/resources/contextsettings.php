 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class contextsettings extends \qtype_lti\local\ltiservice\resource_base {
     ...
     *
     * @param qtype_lti\local\ltiservice\response $response  Response object for this request.
     */
    public function execute($response) {
        ...
        if (!$ok) {
            ...
            $systemsetting = null;
            $contextsettings = qtype_lti_get_tool_settings($this->get_service()->get_tool_proxy()->id, $contextid);
            if (!empty($bubble)) {
                $systemsetting = new systemsettings($this->get_service());
                $systemsetting->params['tool_proxy_id'] = $productcode;
                $systemsettings = qtype_lti_get_tool_settings($this->get_service()->get_tool_proxy()->id);
                if ($bubble == 'distinct') {
            ...
            if ($response->get_request_method() == 'GET') {
                ...
                if ($ok) {
                    qtype_lti_set_tool_settings($settings, $this->get_service()->get_tool_proxy()->id, $contextid);
                } else {
