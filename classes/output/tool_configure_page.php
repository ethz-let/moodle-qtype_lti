...
class tool_configure_page implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $keyhelp = new help_icon('resourcekey', 'qtype_lti');
        $secrethelp = new help_icon('password', 'qtype_lti');

        $url = new moodle_url('/question/type/lti/typessettings.php', array('sesskey' => sesskey(), 'returnto' => 'toolconfigure'));
        $data->configuremanualurl = $url->out();
        $url = new moodle_url('/admin/settings.php?section=qtypesettinglti');
        $data->managetoolsurl = $url->out();
        $url = new moodle_url('/question/type/lti/toolproxies.php');
