...
/**
 * Privacy Subsystem implementation for qtype_lti.
 *
 * @package qtype_lti
 * @copyright 2019 ETH Zurich
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
namespace qtype_lti\privacy;
...
class provider implements \core_privacy\local\metadata\provider, \core_privacy\local\request\plugin\provider {
     ...
     *        a reference to the collection to use to store the metadata.
     * @return collection the updated collection of metadata items.
     */
    public static function get_metadata(collection $items): collection {
        $items->add_external_location_link('qtype_lti_provider',
                                        ['userid' => 'privacy:metadata:userid', 'username' => 'privacy:metadata:username',
                                            'useridnumber' => 'privacy:metadata:useridnumber',
                                            'firstname' => 'privacy:metadata:firstname', 'lastname' => 'privacy:metadata:lastname',
                                            'fullname' => 'privacy:metadata:fullname', 'email' => 'privacy:metadata:email',
                                            'role' => 'privacy:metadata:role', 'courseid' => 'privacy:metadata:courseid',
        ...
        $items->add_database_table('qtype_lti_submission',
        ...
        $items->add_database_table('qtype_lti_tool_proxies',
                                ['name' => 'privacy:metadata:lti_tool_proxies:name',
                                    'createdby' => 'privacy:metadata:createdby', 'timecreated' => 'privacy:metadata:timecreated',
                                    'timemodified' => 'privacy:metadata:timemodified'], 'privacy:metadata:lti_tool_proxies');

        $items->add_database_table('qtype_lti_types',
                                ['name' => 'privacy:metadata:lti_types:name', 'createdby' => 'privacy:metadata:createdby',
                                    'timecreated' => 'privacy:metadata:timecreated',
                                    'timemodified' => 'privacy:metadata:timemodified'], 'privacy:metadata:lti_types');
    ...
    public static function get_contexts_for_userid(int $userid): contextlist {
        ...
        $params = ['modname' => 'qtype_lti', 'contextlevel' => CONTEXT_MODULE, 'userid' => $userid];
     ...
     *        a list of contexts approved for export.
     */
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        if ($cm = get_coursemodule_from_id('lti', $context->instanceid)) {
            $DB->delete_records('qtype_lti_submission', ['ltiid' => $cm->instance]);
        }
    }
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            ...
            $DB->delete_records('qtype_lti_submission', ['ltiid' => $instanceid, 'userid' => $userid]);
        }
    }
    ...
    protected static function export_user_data_lti_submissions(approved_contextlist $contextlist) {
        ...
        $cmids = array_reduce($contextlist->get_contexts(),
                            function ($carry, $context) {
        ...
        $recordset = $DB->get_recordset_select('qtype_lti_submission', "ltiid $insql AND userid = :userid", $params,
                                            'dateupdated, id');
        self::recordset_loop_and_export($recordset, 'ltiid', [],
                                        function ($carry, $record) use ($user, $ltiidstocmids) {
                                            $carry[] = ['gradepercent' => $record->gradepercent,
    ...
    protected static function export_user_data_lti_types(approved_contextlist $contextlist) {
        ...
        $courseids = array_reduce($contextlist->get_contexts(),
                                function ($carry, $context) {
        ...
        $params = array_merge($inparams, ['userid' => $user->id]);
        $ltitypes = $DB->get_recordset_select('qtype_lti_types', "course $insql AND createdby = :userid", $params,
                                            'timecreated ASC');
        self::recordset_loop_and_export($ltitypes, 'course', [],
                                        function ($carry, $record) {
                                            $context = \context_course::instance($record->course);
                                            $options = ['context' => $context];
                                            $carry[] = ['name' => format_string($record->name, true, $options),
                                        ...
                                        function ($courseid, $data) {
                                            ...
                                            $finaldata = (object)['qtype_lti_types' => $data];
                                            writer::with_context($context)->export_data([], $finaldata);
                                        });
    }
    ...
    protected static function export_user_data_lti_tool_proxies(approved_contextlist $contextlist) {
        ...
        $systemcontexts = array_filter($contextlist->get_contexts(),
                                    function ($context) {
        ...
        $data = [];
        $ltiproxies = $DB->get_recordset('qtype_lti_tool_proxies', ['createdby' => $user->id], 'timecreated ASC');
        foreach ($ltiproxies as $ltiproxy) {
            $data[] = ['name' => format_string($ltiproxy->name, true, $systemcontext),
                'createdby' => transform::user($ltiproxy->createdby), 'timecreated' => transform::datetime($ltiproxy->timecreated),
                'timemodified' => transform::datetime($ltiproxy->timemodified)];
        }
        $ltiproxies->close();

        $finaldata = (object)['qtype_lti_tool_proxies' => $data];
     ...
     *        The record key to determine when to export.
     * @param mixed $initial
     *        The initial data to reduce from.
     * @param callable $reducer
     *        The function to return the dataset, receives current dataset, and the current record.
     * @param callable $export
     *        The function to export the dataset, receives the last value from $splitkey and the dataset.
     * @return void
     */
    protected static function recordset_loop_and_export(\moodle_recordset $recordset, $splitkey, $initial, callable $reducer,
                                                        callable $export) {
        $data = $initial;
        $lastid = null;
