...
/**
 * Standard Ajax wrapper for Moodle. It calls the central Ajax script, which can
 * call any existing webservice using the current session. In addition, it can
 * batch multiple requests and return multiple responses.
 *
 * @module qtype_lti/tool_configure_controller
 ...
 */
define(
        [ 'jquery', 'core/ajax', 'core/notification', 'core/templates',
                'qtype_lti/events', 'qtype_lti/keys', 'qtype_lti/tool_type',
                'qtype_lti/tool_proxy', 'core/str' ],
        function ($, ajax, notification, templates, ltiEvents, KEYS, toolType,
            ...
            var showCartridgeRegistration = function (url) {
                ...
                getCartridgeRegistrationContainer().find(
                        SELECTORS.CARTRIDGE_REGISTRATION_FORM).attr(
                        'data-cartridge-url', url);
            ...
            /**
             * JAWS does not notice visibility changes with aria-live. Remove
             * and add the content back to force it to read it out. This
             * function can be removed once JAWS supports visibility.
             *
            ...
            /**
             * Display the registration feedback alert and hide the other
             * panels.
             *
            ...
            var reloadToolList = function () {
                ...
                $.when(toolType.query(), toolProxy.query({
                    'orphanedonly' : true
                })).done(function (types, proxies) {
                    templates.render('qtype_lti/tool_list', {
                        tools : types,
                        proxies : proxies
            ...
            /**
             * Trigger appropriate registration process process for the user
             * input URL. It can either be a cartridge or a registration url.
             *
             * @method addTool
             * @private
            ...
            var addTool = function () {
                ...
                promise.done(function (result) {
                    if (result.iscartridge) {
                        ...
                        $(document).trigger(
                                ltiEvents.START_CARTRIDGE_REGISTRATION, url);
                    } else {
                        $(document).trigger(
                                ltiEvents.START_EXTERNAL_REGISTRATION, {
                                    url : url
                                });
                    }
                });
                ...
                promise.fail(function () {
                    str.get_string('errorbadurl', 'qtype_lti').done(
                            function (s) {
                                $(document).trigger(
            ...
            var registerEventListeners = function () {
                ...
                $(document).on(ltiEvents.START_EXTERNAL_REGISTRATION,
                        function () {
                ...
                $(document).on(ltiEvents.STOP_EXTERNAL_REGISTRATION,
                        function () {
                            showToolList();
                            showRegistrationChoices();
                        });

                $(document).on(ltiEvents.START_CARTRIDGE_REGISTRATION,
                        function (event, url) {
                            showCartridgeRegistration(url);
                        });

                $(document).on(
                        ltiEvents.STOP_CARTRIDGE_REGISTRATION,
                        function () {
                            getCartridgeRegistrationContainer().find(
                                    SELECTORS.CARTRIDGE_REGISTRATION_FORM)
                                    .removeAttr('data-cartridge-url');
                            showRegistrationChoices();
                        });

                $(document).on(ltiEvents.REGISTRATION_FEEDBACK,
                        function (event, data) {
            ...
            return /** @alias module:qtype_lti/cartridge_registration_form */
            {
