 ...
 *
 * See template: qtype_lti/external_registration
 *
 * @module qtype_lti/external_registration
 ...
 * @since 3.6
 */
define(['jquery', 'core/ajax', 'core/notification', 'core/templates', 'qtype_lti/events',
        'qtype_lti/tool_proxy', 'qtype_lti/tool_type', 'qtype_lti/keys', 'core/str'],
        function($, ajax, notification, templates, ltiEvents, toolProxy, toolType, KEYS, str) {
            ...
            /**
             * Return the container that holds all elements for the external
             * registration, including the cancel button and the iframe.
             *
            ...
            /**
             * Return the container that holds the external registration page template.
            ...
            /**
             * Return the container that holds the elements for displaying the list of
             * capabilities that this tool type requires. This container wraps the
             * loading indicator and the template container.
             *
            ...
            /**
             * Return the container that holds the template that lists the capabilities
             * that the tool type will require.
             *
            ...
            /**
             * Triggers a visual indicator to show that the capabilities section is
             * loading.
             *
            ...
            /**
             * Removes the visual indicator that shows the capabilities section is
             * loading.
             *
            ...
            /**
             * Checks if this process has created a tool proxy within Moodle yet.
             *
            ...
            /**
             * Gets the external registration request required to be sent to the
             * external registration page using a form.
             *
             * See qtype_lti/tool_proxy_registration_form template.
             *
            ...
            var getRegistrationRequest = function(id) {
                var request = {
                    methodname: 'qtype_lti_get_tool_proxy_registration_request',
                    args: {
                        id: id
                    }
                };
            ...
            /**
             * Cancel an in progress external registration. This will perform any
             * necessary clean up of tool proxies and return the page section back to
             * the home section.
             *
            ...
            var cancelRegistration = function() {
                ...
                promise.done(function() {
                    ...
                    finishExternalRegistration();
                    stopLoadingCancel();
                    str.get_string('failedtodeletetoolproxy', 'qtype_lti').done(function(s) {
                        var feedback = {
                            message: s,
                            error: true
                        };
            ...
            /**
             * Load the external registration template and render it in the DOM and
             * display it.
             *
            ...
            var renderExternalRegistrationWindow = function(registrationRequest) {
                var promise = templates.render('qtype_lti/tool_proxy_registration_form', registrationRequest);
            ...
            /**
             * Send a request to Moodle server to set the state of the tool type to
             * configured (active).
             *
             * @method setTypeStatusActive
             * @private
             * @param {Object}
             *            typeData A set of data representing a type, as returned by a
             *            request to get a type from the Moodle server.
             * @return {Promise} jQuery Deferred object
             */
            ...
            /**
             * Render and display an agreement page for the user to acknowledge the list
             * of capabilities (groups of data) that the external tool requires in order
             * to work. If the user agrees then we will activate the tool so that it is
             * immediately available. If they don't agree then the tool remains in a
             * pending state within Moodle until agreement is given.
             *
            ...
            var promptForToolTypeCapabilitiesAgreement = function(typeData) {
                var promise = $.Deferred();
                templates.render('qtype_lti/tool_type_capabilities_agree', typeData).done(function(html, js) {
                    ...
                    // The user agrees to allow the tool to use the groups of data so we
                    // can go
                    // ahead and activate it for them so that it can be used straight
                    // away.
                    ...
                    // The user declines to let the tool use the data. In this case we
                    // leave
                    // the tool as pending and they can delete it using the main screen
                    // if they
                    // wish.
            ...
            /**
             * Send a request to the Moodle server to create a tool proxy using the
             * registration URL the user has provided. The proxy is required for the
             * external registration page to work correctly.
             *
             * After the proxy is created the external registration page is rendered
             * within an iframe for the user to complete the registration in the
             * external page.
             *
             * If the tool proxy creation fails then we redirect the page section back
             * to the home section and display the error, rather than rendering the
             * external registration page.
             *
             * @method createAndRegisterToolProxy
             * @private
             * @param {String}
             *            url Tool registration URL to register
             * @return {Promise} jQuery Deferred object
             */
            var createAndRegisterToolProxy = function(url) {
                ...
                if (!url || url === "") {
                    ...
                    // A tool proxy needs to exist before the external page is rendered
                    // because
                    // the external page sends requests back to Moodle for information
                    // that is stored
                    // in the proxy.
                    toolProxy.create({regurl: url})
                        .done(function(result) {
                                // Note that it's a new proxy so we will always clean it
                                // up.
                                setProxyAsNew();
            ...
            var registerProxy = function(id) {
                ...
                setToolProxyId(id);
                // There is a specific set of data needed to send to the external
                // registration page
            ...
            var registerEventListeners = function() {
                ...
                // child iframe and parent windows. There is no other way to
                // communicate.
                //
                // This function gets called by the moodle page that received the
                // redirect
                // from the external registration page and handles the external page's
                // returned
                // parameters.
                //
                // See AMD module qtype_lti/external_registration_return.
                window.triggerExternalRegistrationComplete = function(data) {
                    ...
                    if (data.status == "success") {
                        str.get_string('successfullycreatedtooltype', 'qtype_lti').done(function(s) {
                            feedback.message = s;
                        }).fail(notification.exception);
                        // Trigger appropriate events when we've completed the necessary
                        // requests.
                        promise.done(function() {
                            finishExternalRegistration();
                            $(document).trigger(ltiEvents.REGISTRATION_FEEDBACK, feedback);
                        ...
                        if (hasCreatedToolProxy()) {
                            ...
                            // We need the list of types that are linked to this proxy.
                            // We're assuming it'll
                            // only be one because this process creates a one-to-one
                            // type->proxy.
                            toolType.getFromToolProxyId(proxyId).done(function(types) {
                                if (types && types.length) {
                                    // There should only be one result.
                                    var typeData = types[0];
                                    // Check if the external tool required access to any
                                    // Moodle data (users, courses etc).
                                    if (typeData.hascapabilitygroups) {
                                        // If it did then we ask the user to agree to
                                        // those groups before the type is
                        ...
                        feedback.error = true;
                        // Cancel registration to clean up any proxies and tools that
                        // were created.
                        promise.done(function() {
            ...
            return /** @alias module:qtype_lti/external_registration */ {
                /**
